...
((window) => {
  const { ObjectEntries, ArrayBuffer, Uint8Array, TypeError, Symbol } =
  ...
  class Body extends BufferStream {
    ...
    constructor(object) {
      ...
      if (typeof object === "string") {
        ...
        try {
          JSON.parse(this.#writeObject);
          this.#isJson = true;
        } catch {}

        // Encode string.
        this.#writeObject = encode(object);
        this.#writeType = "string";
    ...
    guessContentType() {
      ...
      switch (this.#writeType) {
        ...
        case "file": {
          ...
          const matches = this.#filepath.match(/(?<=\.)[^.]+$/);
          if (matches) {
            ext = matches[0];
          }
